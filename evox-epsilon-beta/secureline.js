  ...
  fetch(`https://data.evoxs.xyz/secureline?method=MyChats&username=${localStorage.getItem("t50-username")}&recipient_username=${whoto}&password=${atob(localStorage.getItem("t50pswd"))}`)
    ...
    .then(messages => {
      ...
      if (jsonData && jsonData.messages) {
        ...
        processMessage(messages, element)
        return;
        sortedMessages.forEach(message => {
...
function showchat(element) { // json id=Username
  ...
  function handleGesture() {
    if (touchendX > touchstartX) {
      ...
      if (distance > 250) { // Detect a swipe if the distance is greater than 250 pixels
  ...
  document.getElementById("messages-container").innerHTML = `
    <p class='centered-text'>Loading Messages
  ...
  const pfp = document.getElementById(`${element.id}-pfp-secureline`);
  document.getElementById("secureline-img-chat").src = pfp.src;
  document.getElementById("chat-username").innerHTML = element.id;
  const createChatPromise = fetch(`https://data.evoxs.xyz/secureline?method=CreateChat&username=${localStorage.getItem("t50-username")}&recipient_username=${element.id}&password=${atob(localStorage.getItem("t50pswd"))}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
    ...
    .then(data => {
      if (data === "Chat Exists.") {
        ...
        return fetch(`https://data.evoxs.xyz/secureline?method=MyChats&username=${localStorage.getItem("t50-username")}&recipient_username=${element.id}&password=${atob(localStorage.getItem("t50pswd"))}`)
          .then(response => {
            if (!response.ok) {
              throw new Error(`HTTP error! Status: ${response.status}`);
            }
          ...
          .then(messages => {
            try {
              JSON.parse(messages); // This is just an integrity check
            } catch (error) {
              console.error("Possible Account Verification Error:", messages);
              return;
            }
        ...
        document.getElementById("messages-container").innerHTML = `
          <p class='centered-text'>Chat Hasn't Been Created.<button style="margin-top: 20px" id="submit" onclick="create_chat()" class="transparent-button">Create Chat</button></p>`;
