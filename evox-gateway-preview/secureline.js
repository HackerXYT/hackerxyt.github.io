...
function send_message() {
    ...
    if (message.value != "") {
        ...
        fetch(`http://192.168.1.21:9000/secureline?method=SendMessage&username=${localStorage.getItem("t50-username")}&recipient_username=${recipient}&message=${message.value}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                if (data === `Message Sent To ${recipient}`) {
                    ...
                    fetch(`http://192.168.1.21:9000/secureline?method=MyChats&username=${localStorage.getItem("t50-username")}&recipient_username=${recipient}`)
                        .then(response => {
                            if (!response.ok) {
                                throw new Error(`HTTP error! Status: ${response.status}`);
                            }
...
function reload_chat(whoto) {
  reloading = setInterval(function () {
    ...
    fetch(`http://192.168.1.21:9000/secureline?method=MyChats&username=${localStorage.getItem("t50-username")}&recipient_username=${whoto}`)
      .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
...
function showchat(element) {
  ...
  fetch(`http://192.168.1.21:9000/secureline?method=CreateChat&username=${localStorage.getItem("t50-username")}&recipient_username=${element.id}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
    ...
    .then(data => {
      if (data === "Chat Exists.") {
        ...
        fetch(`http://192.168.1.21:9000/secureline?method=MyChats&username=${localStorage.getItem("t50-username")}&recipient_username=${element.id}`)
          .then(response => {
            if (!response.ok) {
              throw new Error(`HTTP error! Status: ${response.status}`);
            }
...
function create_chat() {
  ...
  fetch(`http://192.168.1.21:9000/secureline?method=CreateChat&username=${localStorage.getItem("t50-username")}&recipient_username=${sessionStorage.getItem("current_sline")}&createnew=true`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
function getFriends(pre) {
  ...
  function confirmdelete() {
    ...
    fetch(`http://192.168.1.21:9000/secureline?method=DeleteMessage&username=${localStorage.getItem("t50-username")}&recipient_username=${sessionStorage.getItem("current_sline")}&whosentit=me&message=${message}`)
      .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
