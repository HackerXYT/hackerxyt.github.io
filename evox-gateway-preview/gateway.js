  ...
  if (lg_status === "true") {
    ...
    $("#container").fadeOut("slow", function () {
      ...
      if (username != null && email != null) {
        ...
        $("#loading").fadeIn("slow")
        $("#stuck").fadeOut("slow")
        fetch(`http://192.168.1.21:9000/accounts?applications=get&email=${localStorage.getItem("t50-email")}`)
          .then(response => {
            if (!response.ok) {
...
function docready() {
  ...
  if (loggedin != null && autologin === "true") {
    const url = `http://192.168.1.21:9000/accounts?email=${loggedin}&password=${pswd}&autologin=true&ip=${localStorage.getItem("IPV4")}`;
    ...
    fetch(url)
      ...
      .then(data => {
        if (data.includes("Credentials Correct")) {
          ...
          fetch(`http://192.168.1.21:9000/authip?method=get&email=${loggedin}&username=${username}&password=${pswd}&ip=${localStorage.getItem("IPV4")}`)
            .then(response => {
              if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
              }
            ...
            .then(data => {
              if (data === "IP is Mapped") {
                ...
                fetch(`http://192.168.1.21:9000/authip?method=forceadd&email=${loggedin}&username=${username}&password=${pswd}&ip=${localStorage.getItem("IPV4")}`)
                  .then(response => {
                    if (!response.ok) {
                      throw new Error(`HTTP error! Status: ${response.status}`);
                    }
...
function verifycode() {
  ...
  fetch(`http://192.168.1.21:9000/authip?method=add&email=${email}&username=${username}&password=${password}&code=${code}&ip=${localStorage.getItem("IPV4")}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
function login() {
  ...
  console.log(email, "********")
  const url = `http://192.168.1.21:9000/accounts?email=${email}&password=${password}&ip=${localStorage.getItem("IPV4")}`;
...
function reconnect() {
  ...
  fetch("http://192.168.1.21:9000/cron")
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
