...
function set(day, custom) {
  ...
  //https://data.evoxs.xyz/tasco || httpe://192.168.1.21:4000/tasco -> added "e"
  fetch(`https://data.evoxs.xyz/tasco?method=get&username=${username}&day=${dayOfWeek}`)
    .then(response => {
      if (!response.ok) {
  ...
  fetch(`https://data.evoxs.xyz/tasco?method=debts&debts=get&username=${localStorage.getItem("t50-username")}`)
    ...
    .then(data => {
      ...
      for (const [username, value] of Object.entries(users)) {
        ...
        const url = `https://data.evoxs.xyz/profiles?authorize=351c3669b3760b20615808bdee568f33&pfp=${username}`;
        fetch(url)
...
function adddebt() {
  ...
  fetch(`https://data.evoxs.xyz/tasco?method=debts&debts=set&username=${username}&debtcost=${Number(price)}&issuer=${receiver}`)
    .then(response => response.text())
    .then(data => {
      console.log(data)
      document.getElementById("donut-container").innerHTML = `<canvas id="donut" width="250" height="250"></canvas>`
...
function cleardebt() {
  ...
  fetch(`https://data.evoxs.xyz/tasco?method=debts&debts=del&username=${username}&issuer=${receiver}`)
    .then(response => response.text())
    .then(data => {
      console.log(data)
      $('#debts-popup').fadeOut();
      ...
      fetch('https://data.evoxs.xyz/profiles', {
        ...
        .then(data => {
          ...
          if (data === "done") {
            ...
            const url = `https://data.evoxs.xyz/profiles?authorize=351c3669b3760b20615808bdee568f33&pfp=${sessionStorage.getItem("chdebtpfp")}`;
            fetch(url)
              .then(response => response.text())
              .then(prof => {
