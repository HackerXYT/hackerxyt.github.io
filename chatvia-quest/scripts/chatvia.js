...
readTextFile(file, function(text) {
  ...
  if(localStorage.getItem("srv") == srv) {
    if(srv.includes("04")) {
      console.log("Γινεται Χρηση Προεπιλεγμενου Διακομιστη ChatVia Στην Τοποθεσια Κανσας, ΗΠΑ")
    } else {
      console.log("Γινεται Χρηση Μη Προεπιλεγμενου Διακομιστη Αγνωστης Τοποθεσιας. Παραπανω Πληροφοριες:", srv)
    }
  } else {
...
socket.on('message', function(data) {
  ...
  if(text.includes("Καλωσορισατε")) {
    ...
    console.log("Doesnt include welcome")
  }
...
return;
  ...
  <div class="conversation-list">
    ...
    notification_pc(data.user, data.message)
    document.getElementById("updatelastmsg_2ndchat").innerHTML = "τωρα"
	  msgtimeup_for2()
  ...
  for (var key in localStorage) {
    if (key.includes('liked_song_no')) {
      if (localStorage.getItem(key) === 'Οχι Ετοιμο') {
        console.log('The value for ' + key + ' is Οχι Ετοιμο. Setting it to null.');
        localStorage.removeItem(key);
      }
    }
  }
