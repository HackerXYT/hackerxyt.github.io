...
function load_db() {
  ...
  fetch(`https://evox-datacenter.onrender.com/images-database?password=${password}`, {
    method: 'GET',
    headers: {
    ...
    .then(data => {
      ...
      saveImagesToIndexedDB(decodedValues)
        .then(message => {
          console.log(message);
        })
        .catch(error => {
...
function format_db() {
  ...
  if (userInput !== null) {
    let api = "aHR0cDovLzE5Mi4xNjguMS4yNzo0MDAwP3Bhc3N3b3JkPQ=="//4000
    fetch(`https://evox-datacenter.onrender.com/images-database?password=${password}&method=format&format=${userInput}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json', // Modify this based on your API's requirements
      },
